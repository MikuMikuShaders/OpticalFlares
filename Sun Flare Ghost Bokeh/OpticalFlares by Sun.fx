...
// B = Fixed Brightness
float3 GhostFlareParams = float3(6, 10.0, 1);
...
float3 GhostAllOffset = float3(0.1, 0.25, 0.02);

// R = Fixed Brightness
// G = Increase Brightness by ID
float2 GhostAllBrightness = float2(10.0, -0.1);

// RGB color
float3 GhostAllColor = float3(0.5, 0.5, 0.0);
...
float4 GhostParams[16] = { 
	float4(1.0, 1.0, 26.0, 1.0), float4(1.0, 1.0, 11.5, 1.0), float4(0.5, 1.0, 5.0, 1.0), float4(0.39, 1.0, 4.02, 1.0),
	float4(0.8, 1.0, 3.3, 0.5), float4(0.7, 1.0, 2.75, 0.8), float4(0.9, 1.0, 2.23, 1.0), float4(0.5, 1.0, 1.42, 1.0), 
	float4(0.5, 1.0, 1.38, 1.0), float4(0.4, 1.0, 1.45, 1.0), float4(1.0, 1.0, 1.2, 1.0), float4(0.4, 1.0, 1.35, 2.0),
	float4(0.25, 1.0, 0.99, 0.2), float4(0.3, 1.0, 1.2, 1.0), float4(1.0, 1.0, 1.0, 1.0), float4(0.1, 1.0, 1.1, 0.5)
};
